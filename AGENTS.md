...
| `docs/architecture.md`       | Qué significa "hacer un buen trabajo" en este proyecto, incl. dependencias pendientes en otros repos | Antes de implementar |
...
| `docs/security-scope.md`     | Límites de autorización/alcance (sesión, OIDC, credenciales de servicio, Broker) | Antes de tocar login, sesión, o credenciales hacia `ms-usuarios`/Broker |
...
- **Si no sabes algo, busca en `docs/`** antes de inventarlo — en particular,
  el contrato del Broker (exchanges/colas/permisos) es de `broker/`, no se
  re-deriva aquí (ver `docs/architecture.md`).
- **Antes de tocar login/sesión, la credencial compartida con
